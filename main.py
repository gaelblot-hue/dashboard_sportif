...
# ============================================================
UPSTASH_URL   = os.getenv('UPSTASH_REDIS_REST_URL', 'https://live-marlin-35304.upstash.io')
UPSTASH_TOKEN = os.getenv('UPSTASH_REDIS_REST_TOKEN', 'AYnoAAIncDE0NWI3NWZmMzliMTU0Y2Y4YTQ2YTAwODE2MTM3ODNkZHAxMzUzMDQ')
...
def chat():
    ...
    system_prompt = (
        "Tu es RADAR, l'IA experte en paris sportifs de Gael. "
        "Tu connais Gael et tu te souviens de vos conversations precedentes. "
        "Sois precis, professionnel, toujours en francais. "
        f"Bankroll dashboard : {bankroll_info}. "
        f"Bookmakers de Gael : {bookmakers_str} - compare toujours les cotes sur ces 4 bookmakers. "
        "Sources : BallDontLie (NBA), API-Sports (Football, Basket, Tennis). "
        "Pour Over/Under, precise toujours OVER ou UNDER avec le total exact. "
        "Si une image est partagee, analyse-la en detail pour en extraire les infos utiles (cotes, stats, scores). "
        "Tiens compte de la forme recente des equipes (5 derniers matchs) dans tes analyses. "
        "Pour les alertes temps reel : signale immediatement si tu detectes une value bet sur un match en cours."
    )
...
def analyse_rafale():
    ...
    contenu = [
        {
            "type": "text",
            "text": (
                f"Match analysé : {match_label}. "
                "Tu reçois des captures groupées (H2H, Séries, Cotes, Stats). "
                "Calcule l'Edge (seuil 15%) et donne une analyse Radar V6. "
                "Réponds UNIQUEMENT en JSON avec les clés : "
                "match, edge_pct, value_bet (bool), pari_suggere, cote_cible, "
                "confiance_pct, resume, signaux (liste)."
            )
        }
    ]
    ...
    try:
        ...
        try:
            ...
            # Si pas du JSON propre, on retourne le texte brut
