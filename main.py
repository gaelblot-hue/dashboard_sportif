...
def fetch_football_today(league_id):
    ...
    return apisports_get("/fixtures", {"league": league_id, "date": today, "season": 2024}, "football")
...
def fetch_basket_today(league_id):
    today = datetime.now().strftime("%Y-%m-%d")
    return apisports_get("/games", {"league": league_id, "date": today, "season": "2024-2025"}, "basketball")
...
def test():
    ...
    try:
        r = requests.get("https://v3.football.api-sports.io/fixtures",
            headers={"x-apisports-key": APISPORTS_KEY},
            params={"league": 39, "date": today, "season": 2024}, timeout=5)
