...
def calculer_mise(bankroll_disponible, confiance):
    """Grille de mise selon niveau de confiance (proposée par le bot)."""
    pourcentages = {7: 0.02, 8: 0.04, 9: 0.06, 10: 0.08}
    # Confiance en % → convertir en échelle /10
    if isinstance(confiance, float) and confiance <= 1:
    ...
    if confiance >= 90:
        ...
        pct = pourcentages.get(int(confiance), 0.02)
    return round(bankroll_disponible * pct, 2)

def calculer_edge(prob_reelle, cote_bookmaker):
    """
    Formule Edge améliorée avec log-odds.
    prob_reelle   : probabilité réelle estimée (0-1)
    cote_bookmaker: cote décimale du bookmaker (ex: 2.10)
    Retourne : edge en %, niveau de confiance, mise recommandée
    """
    import math
    if not prob_reelle or not cote_bookmaker or cote_bookmaker <= 1:
        return None

    prob_bookmaker = 1 / cote_bookmaker

    # Edge classique
    edge_classique = ((prob_reelle - prob_bookmaker) / prob_bookmaker) * 100

    # Edge log-odds (plus précis sur les extrêmes)
    try:
        lo_reel = math.log(prob_reelle / (1 - prob_reelle))
        lo_book = math.log(prob_bookmaker / (1 - prob_bookmaker))
        edge_logodds = ((lo_reel - lo_book) / abs(lo_book)) * 100
    except Exception:
        edge_logodds = edge_classique

    # Edge combiné (moyenne pondérée)
    edge_final = round((edge_classique * 0.6 + edge_logodds * 0.4), 2)
    ...
    return {
        "edge_pct": edge_final,
        "edge_classique": round(edge_classique, 2),
        "edge_logodds": round(edge_logodds, 2),
        "confiance_pct": confiance,
        "risque": risque,
        "value_bet": edge_final >= 15
    }

# Bankroll Gaël par bookmaker (mise à jour manuelle ou via bankroll endpoint)
BOOKMAKERS_BANKROLL = {
    "Betclic":  14.99,
    "Winamax":  28.88,
    "Betify":   58.86,
...
BANKROLL_TOTALE = sum(BOOKMAKERS_BANKROLL.values())  # 145.21€
...
def chat():
    ...
    # Récupération cotes temps réel si possible
    cotes_reelles = ""
    if message:
        for sport_key in ODDS_SPORTS.keys():
            if sport_key in message.lower() or any(s in message.lower() for s in ['nba','foot','tennis','basket']):
                odds_data = fetch_odds_api(sport_key, message)
                if odds_data and isinstance(odds_data, dict):
                    cotes_reelles = f"COTES REELLES ODDSAPI : {json.dumps(odds_data, ensure_ascii=False)}"
                    break

    # Calcul bankroll réelle par bookmaker
    bookmakers_bankroll_str = (
        f"Betclic={BOOKMAKERS_BANKROLL['Betclic']}€ | "
        f"Winamax={BOOKMAKERS_BANKROLL['Winamax']}€ | "
        f"Betify={BOOKMAKERS_BANKROLL['Betify']}€ | "
        f"Mystake={BOOKMAKERS_BANKROLL['Mystake']}€ | "
        f"TOTAL={BANKROLL_TOTALE}€"
    )

    system_prompt = (
        "Tu es RADAR V6, l'IA experte en paris sportifs de Gael. Tu es son assistant personnel de confiance. "
        "Tu connais Gael et tu te souviens de vos conversations precedentes. "
        "REGLES ABSOLUES - NE JAMAIS ENFREINDRE : "
        "1. NE JAMAIS inventer des cotes - utilise UNIQUEMENT les cotes visibles dans les images ou fournies par les APIs. "
        "2. NE JAMAIS inventer des stats ou des scores - uniquement ce qui est visible dans les captures ou les donnees reelles. "
        "3. Si tu n as pas les donnees, dis-le clairement plutot qu inventer. "
        "ANALYSE VALUE BET : "
        "- Edge minimum requis : 15% pour valider un value bet. "
        "- FORMULE EDGE AMELIOREE (log-odds) : "
        "  Etape 1 : Edge classique = ((Prob_reelle - Prob_bookmaker) / Prob_bookmaker) x 100 "
        "  Etape 2 : Edge log-odds = ((ln(p/(1-p)) - ln(q/(1-q))) / |ln(q/(1-q))|) x 100 "
        "  Etape 3 : Edge final = (Edge_classique x 0.6) + (Edge_log-odds x 0.4) "
        "  Utilise toujours cette formule combinee pour plus de precision. "
        "- VERDICT final toujours : JOUER ou NE PAS JOUER, jamais ambigu. "
        "GESTION BANKROLL GAEL : "
        f"Bankroll par bookmaker : {bookmakers_bankroll_str}. "
        f"Bankroll dashboard : {bankroll_info}. "
        "GRILLE DE MISE (% de la bankroll TOTALE de 145.21€) : "
        "- Confiance 60-69% : mise 2% = 2.90€ - RISQUE FAIBLE. "
        "- Confiance 70-79% : mise 4% = 5.81€ - RISQUE MOYEN. "
        "- Confiance 80-89% : mise 6% = 8.71€ - RISQUE ELEVE. "
        "- Confiance 90-100% : mise 8% = 11.62€ - RISQUE MAXIMUM. "
        "Indique TOUJOURS sur quel bookmaker placer la mise selon la meilleure cote disponible. "
        "BOOKMAKERS : "
        f"{bookmakers_str} - Compare TOUJOURS les cotes sur ces 4 bookmakers et indique le meilleur. "
        "SOURCES DE DONNEES : BallDontLie (NBA), API-Sports (Football, Basket, Tennis). "
        "FORMAT DE REPONSE : "
        "- Verdict : JOUER / NE PAS JOUER en gras en premier. "
        "- Bookmaker recommande avec la meilleure cote. "
        "- Mise recommandee en euros selon la bankroll et la confiance. "
        "- Niveau de risque : FAIBLE / MOYEN / ELEVE. "
        "- Pour Over/Under : precise OVER ou UNDER avec le total exact et la cote. "
        "- Si image partagee : extrait toutes les stats, cotes, scores, series visibles. "
        "- Tiens compte de la forme recente (5 derniers matchs), H2H, domicile/exterieur. "
        "- Signale immediatement tout value bet detecte sur un match en cours. "
        "- Memorise les analyses precedentes pour affiner tes predictions futures. "
        "Sois precis, direct, professionnel. Toujours en francais. ""Utilise ### pour les titres de section et ** pour les mots importants. ""Structure toujours ta reponse avec des sections claires. "f"{cotes_reelles}"
    )
    ...
    contenu = [
        {
            ...
            "text": (
                ...
                "\"risque\": \"FAIBLE\" ou \"MOYEN\" ou \"ELEVE\" ou \"MAXIMUM\", "
                "\"mise_recommandee\": string, "
                "\"resume\": string, "
                "\"signaux\": [liste de strings]}. "
                "REGLES EDGE AMELIORE : "
                "1. Calcule Edge classique = ((Prob_reelle - 1/cote) / (1/cote)) x 100. "
                "2. Calcule Edge log-odds = ((ln(p/(1-p)) - ln(q/(1-q))) / |ln(q/(1-q))|) x 100. "
                "3. Edge final = (classique x 0.6) + (log-odds x 0.4). "
                "GRILLE DE MISE sur bankroll totale 145.21€ : "
                "confiance 60-69% = 2% = 2.90€ FAIBLE | "
                "confiance 70-79% = 4% = 5.81€ MOYEN | "
                "confiance 80-89% = 6% = 8.71€ ELEVE | "
                "confiance 90-100% = 8% = 11.62€ MAXIMUM. "
