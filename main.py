...
def chat():
    ...
    # Charge historique depuis fichier si session vide
    if not CHAT_HISTORY:
        CHAT_HISTORY = load_chat_history()

    bankroll = load_bankroll()
    dispo = bankroll.get('disponible', 0)
    bankroll_info = f"{dispo}€ sur le dashboard" if dispo > 0 else "0€ sur le dashboard (Gaël a de la bankroll sur ses bookmakers)"
    ...
    system_prompt = (
        ...
        "Sois bref, precis, professionnel, toujours en francais. "
        ...
        "Important : pour Over/Under, precise toujours OVER ou UNDER avec le total exact."
    )
