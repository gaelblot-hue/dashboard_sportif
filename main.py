...
# Clés optionnelles — plus utilisées activement mais gardées pour compatibilité /health et /test
...
CACHE_DURATIONS = {
    ...
    "odds":       4  * 3600,  # 4h     — cotes
    "weather":    30 * 60,    # 30 min — météo
    "tennis_key": 24 * 3600,  # 24h    — tournoi tennis actif
}
...
def fetch_sofascore_events(sport_id):
    """
    Récupère les prochains matchs + cotes via l'API interne SofaScore.
    Zéro clé API, 100% HTTP requests.
    """
    ...
    # Déterminer le sport SofaScore
    sport_slug = "football"
    if sport_id in ["nba", "euroleague", "eurocup"]:
    ...
    try:
        ...
        for event in events:
            ...
            # Récupérer les cotes SofaScore pour cet event
...
def fetch_sofascore_odds(event_id):
    ...
    Retourne une liste de cotes formatée pour Radar V6.
    """
    if not event_id:
        return []
    try:
        ...
        for market in markets:
            if market.get("marketName") in ["Full time", "Moneyline", "1X2", "Match Winner"]:
                ...
                for choice in choices:
                    ...
                    try:
                        ...
                        val = None
                    if name in ["1", "Home", "W1"]:
                        home_cote = val
                    elif name in ["2", "Away", "W2"]:
                        away_cote = val
                    elif name in ["X", "Draw"]:
                        draw_cote = val
...
# ============================================================
# FLASHSCORE — Scores live + cotes (zéro clé API)
# ============================================================
FLASHSCORE_HEADERS = {
    "User-Agent": "Mozilla/5.0 (Linux; Android 12; Pixel 6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36",
    "Accept": "*/*",
    ...
    "X-Fsign": "SW9D1eZo",  # Token public Flashscore — stable depuis des années
}

# Mapping sport_id → identifiants Flashscore
FLASHSCORE_SPORTS = {
...
def parse_flashscore_feed(raw_text):
    """
    Parse le format texte propriétaire Flashscore.
    Chaque ligne : ¬CHAMP¬VALEUR¬CHAMP¬VALEUR...
    Les blocs matchs commencent par AA¬
    """
    matchs = []
    if not raw_text:
        return matchs

    # Séparer les blocs par le séparateur Flashscore
    blocs = raw_text.split('~')
    for bloc in blocs:
        if not bloc.strip():
            continue
        # Parser chaque champ clé/valeur
        parts = bloc.split('¬')
        data = {}
        i = 0
        while i < len(parts) - 1:
        ...
        # Status : 1=scheduled, 2=live, 4=mi-temps, 5=terminé
        is_live = status_code in ['2', '3', '4', '6', '7']
        is_finished = status_code in ['5', '100']

        if is_finished:
            continue  # On skip les matchs terminés
...
def fetch_flashscore_live():
    """
    Récupère TOUS les matchs live toutes compétitions via le feed Flashscore.
    Endpoint : d.flashscore.com/x/feed/f_1_0_1_en_1
    Format propriétaire texte séparé par ¬
    """
...
def fetch_flashscore_sport(sport_id):
    """
    Récupère les matchs d'un sport/compétition via Flashscore.
    Endpoint : d.flashscore.com/x/feed/f_2_{sport}_{league_id}_en_1
    """
...
def fetch_flashscore_odds(match_id):
    """
    Récupère les cotes Flashscore pour un match donné.
    Endpoint : d.flashscore.com/x/feed/df_od_1_{match_id}
    """
    if not match_id:
        return []
    # Retirer le préfixe fs_ si présent
    clean_id = match_id.replace("fs_", "")
    try:
        url = f"https://d.flashscore.com/x/feed/df_od_1_{clean_id}"
        r = requests.get(url, headers=FLASHSCORE_HEADERS, timeout=8)
        if r.status_code != 200:
            return []

        # Parser les cotes depuis le format texte
        text = r.text
        cotes = []
        parts = text.split('¬')
        data = {}
        ...
        # Les cotes sont souvent dans XA (home), XB (draw), XC (away)
        home_cote = data.get('XA') or data.get('XH')
        draw_cote = data.get('XB') or data.get('XD')
        away_cote = data.get('XC') or data.get('XA2')

        try:
            home_cote = float(home_cote) if home_cote else None
            draw_cote = float(draw_cote) if draw_cote else None
            away_cote = float(away_cote) if away_cote else None
        except (ValueError, TypeError):
            home_cote = draw_cote = away_cote = None
        ...
        print(f"Flashscore odds erreur ({match_id}): {e}")
        return []

def merge_matchs(sofascore_matchs, flashscore_matchs):
    """
    Fusionne les matchs SofaScore et Flashscore.
    - Préfère le score live de Flashscore s'il est disponible
    - Complète les cotes manquantes de SofaScore avec celles de Flashscore
    - Déduplique par nom d'équipe (fuzzy match simplifié)
    """
    ...
    # Index des matchs SofaScore par nom d'équipe (lowercase)
    index = {}
    ...
    for fs_match in flashscore_matchs:
        ...
        if key in index:
            # Match trouvé — enrichir avec données Flashscore
            i = index[key]
            # Score live prioritaire depuis Flashscore
            ...
            # Cotes Flashscore si SofaScore n'en a pas
            if not merged[i].get('cotes') and fs_match.get('cotes'):
                merged[i]['cotes'] = fs_match['cotes']
        else:
            # Match non trouvé dans SofaScore — on l'ajoute
...
def scrape_bovada_nba():
    """Récupère les cotes NBA directement depuis Bovada."""
...
# ============================================================
# THESPORTSDB — API gratuite multi-sports (fallback matchs)
# ============================================================
SPORTSDB_KEY = "3"
...
# Mapping sport_id → nom lisible pour le chat
APISPORTS_LEAGUES = {
...
def fetch_sportsdb_events(league_id):
    """Récupère les prochains matchs TheSportsDB — fallback si SofaScore échoue."""
...
# ============================================================
# FETCH SPORTS — SofaScore principal, fallbacks TheSportsDB / Bovada
# ============================================================
def fetch_nba():
    """NBA : SofaScore + Flashscore en parallèle → Bovada → TheSportsDB."""
    cached = get_cache("sport_nba", "scheduled")
    if cached:
        return cached

    # 1. SofaScore + Flashscore en parallèle
    sofa = fetch_sofascore_events("nba")
    flash = fetch_flashscore_sport("nba")
    matchs = merge_matchs(sofa, flash)

    # 2. Fallback Bovada si les deux échouent
    if not matchs:
...
def fetch_football_sport(sport_id):
    """Foot & basket Europe : SofaScore + Flashscore en parallèle → TheSportsDB."""
    ...
    # 2. Fallback TheSportsDB si les deux échouent
    if not matchs:
        league_id = SPORTSDB_LEAGUES.get(sport_id)
        if league_id:
            matchs = fetch_sportsdb_events(league_id)
...
def fetch_basket_sport(sport_id):
    """Basket Europe via SofaScore → TheSportsDB."""
    return fetch_football_sport(sport_id)

def fetch_tennis_sport(sport_id):
    """Tennis via SofaScore → TheSportsDB."""
    return fetch_football_sport(sport_id)
...
def test():
    ...
    try:
        r = requests.get(
            "https://d.flashscore.com/x/feed/f_1_0_1_en_1",
    ...
    try:
        matchs = scrape_bovada_nba()
        results["bovada_nba"] = {"matchs": len(matchs)}
    except Exception as e:
        results["bovada_nba"] = {"erreur": str(e)}
...
@app.route('/test/live')
def test_live():
    """Teste tous les matchs EN DIRECT via Flashscore (toutes compétitions)."""
    try:
...
# ============================================================
# ENDPOINT LIVE GLOBAL — Flashscore toutes compétitions
# ============================================================
@app.route('/radar/live')
def get_live():
    """
    Tous les matchs EN DIRECT en ce moment via Flashscore.
    Pas de filtre sport — tout ce qui est live.
    """
    matchs = fetch_flashscore_live()
    return jsonify({
        "data": matchs,
...
def get_sport(sport_id):
    if sport_id == "nba":
        return jsonify({"data": fetch_nba(), "source": "sofascore+bovada+thesportsdb"})
    elif sport_id in SOFASCORE_TOURNAMENTS or sport_id in SPORTSDB_LEAGUES:
        matchs = fetch_football_sport(sport_id)
        return jsonify({"data": matchs, "source": "sofascore+thesportsdb"})
    else:
        return jsonify({"data": []})

# ============================================================
# ENDPOINT TEST SOFASCORE DIRECT
# ============================================================
@app.route('/radar/sofascore/<sport_id>')
def get_sofascore(sport_id):
    """Endpoint de test SofaScore pour un sport donné."""
...
def get_flashscore(sport_id):
    """Endpoint de test Flashscore pour un sport donné."""
...
def delete_bookmaker():
    ...
    save_bookmakers(bks)
    return jsonify({"status": "deleted", "bookmakers": bks, "total": round(sum(x['solde'] for b in bks for b in [b]), 2)})
...
def chat():
    ...
    # Récupération cotes temps réel si le message parle d'un match
    cotes_reelles = ""
...
def analyse_rafale():
    """
    Reçoit jusqu'à 3 images base64, les optimise,
