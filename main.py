...
def fetch_nba_odds():
    ...
    today = datetime.now().strftime("%Y-%m-%d")
    yesterday = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
    data = bdl_get("/nba/v2/odds", {"date": today, "per_page": 15})
    if not data or not data.get("data"):
        data = bdl_get("/nba/v2/odds", {"date": yesterday, "per_page": 15})
    return data
...
def chat():
    ...
    system_prompt = (
        ...
        "- Formule Edge : ((Probabilite reelle - Probabilite bookmaker) / Probabilite bookmaker) x 100. "
        "- VERDICT final toujours : JOUER ou NE PAS JOUER, jamais ambigu. "
        "GESTION BANKROLL : "
        f"Bankroll disponible : {bankroll_info}. "
        "- Confiance 7/10 ou 70% : mise 2% de la bankroll - RISQUE FAIBLE. "
        "- Confiance 8/10 ou 80% : mise 4% de la bankroll - RISQUE MOYEN. "
        "- Confiance 9/10 ou 90% : mise 6% de la bankroll - RISQUE ELEVE. "
        "- Confiance 10/10 ou 100% : mise 8% de la bankroll - RISQUE MAXIMUM. "
        "BOOKMAKERS : "
...
def analyse_rafale():
    ...
    """
    data = request.get_json()
    images_b64 = data.get('images', [])  # liste de strings base64
    match_label = data.get('match', 'Match inconnu')

    if not images_b64:
        return jsonify({"error": "Aucune image reçue"}), 400
    if len(images_b64) > 6:
        images_b64 = images_b64[:6]
    ...
    try:
        ...
        # Nettoyage robuste et parse JSON
        ...
        print(f"DEBUG rafale raw: {raw_reply[:200]}")
        print(f"DEBUG rafale clean: {clean[:200]}")
