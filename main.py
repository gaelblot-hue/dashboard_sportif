...
def chat():
    ...
    # Récupération forme récente via TheSportsDB
    forme_context = ""
    if message:
        for sport_id, league_id in SPORTSDB_LEAGUES.items():
            if sport_id in message.lower():
                last_events = fetch_sportsdb_last_events(league_id)
                if last_events:
                    forme_context = " FORME RECENTE (TheSportsDB) : " + " | ".join([f"{e['home']} {e['score']} {e['away']} ({e['date']})" for e in last_events[-5:]])
                break
